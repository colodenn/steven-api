...
export async function mineStone(bot: Bot) {
  ...
  const directions = [new Vec3(1, 0, 0), new Vec3(0, 0, 1), new Vec3(-1, 0, 0), new Vec3(0, 0, -1), new Vec3(0, -1, 0)];
  ...
  // Mine the stone
  const { output: mineOutput, success } = await mineBlock(bot, "stone", 3);
